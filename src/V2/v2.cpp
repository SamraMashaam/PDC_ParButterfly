...
int main(int argc, char** argv) {
    ...
    auto local_tips = tip_decomposition(local_graph, full_graph);
    MPI_Barrier(MPI_COMM_WORLD);

    auto local_wings = wing_decomposition(local_graph);
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        // Output combined results (similar to your original output code)
        log_output("\n===== Combined Results =====\n");
        ostringstream oss;
    ...
    // Synchronized output to console (more reliable than file)
    for (int p = 0; p < num_procs; p++) {
        MPI_Barrier(MPI_COMM_WORLD);
        if (rank == p) {
            cout << "\n===== Process " << rank << " Results =====" << endl;
            
            // Per-vertex counts
            cout << "Per-vertex butterfly counts:" << endl;
            for (const auto& [u, cnt] : local_exact_counts.per_vertex) {
                if (cnt > 0) {
                    cout << "Vertex " << safe_label(u, u < local_graph.U.size())
                        << ": " << cnt << endl;
                }
            }

            // Per-edge counts (limited)
            cout << "\nPer-edge butterfly counts (sample):" << endl;
            size_t shown = 0;
            for (const auto& [e, cnt] : local_exact_counts.per_edge) {
                if (shown++ >= 10) break;
                cout << "Edge (" << safe_label(e.first, true) << "," 
                    << safe_label(e.second, false) << "): " << cnt << endl;
            }

            // Tip numbers
            cout << "\nTip numbers:" << endl;
            for (size_t i = 0; i < min(local_tips.size(), local_graph.U.size() + local_graph.V.size()); ++i) {
                if (local_tips[i] > 0) {
                    cout << "Vertex " << safe_label(i, i < local_graph.U.size())
                        << ": " << local_tips[i] << endl;
                }
            }

            cout << "\nExact counting time: " << fixed << setprecision(7) 
                << local_exact_counts.counting_time << " ms" << endl;
        }
        MPI_Barrier(MPI_COMM_WORLD);
    }

    // Only root handles the file output
