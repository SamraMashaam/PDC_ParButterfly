...
ButterflyCounts approximate_count(BipartiteGraph& graph, double sample_prob = 0.1) {
    ...
    for (long long i = 0; i < sample_size; ++i) {
        ...
        if (v1 >= graph.V.size() || v2 >= graph.V.size()) continue;
    
        for (int u2 : graph.V[v1]) {
            if (u2 < 0 || u2 >= graph.U.size()) {
                cerr << "Invalid u2 = " << u2 << " in graph.V[" << v1 << "]\n";
                continue;
            }
...
// Add these helper functions for MPI data distribution
...
void divideGraph(const BipartiteGraph& full_graph, BipartiteGraph& local_graph, 
    ...
    // Calculate ranges for this process
...
int main(int argc, char** argv) {
    ...
    // Process local part of the graph
