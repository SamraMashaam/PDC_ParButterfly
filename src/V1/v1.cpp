...
ButterflyCounts exact_count(BipartiteGraph& graph, const string& aggregation = "auto") {
    ...
    unordered_map<pair<int, int>, vector<pair<int, int>>, PairHash> edge_to_wedges; // Maps edge (u,v) to wedges (u1,u2)
    ...
    for (size_t u = 0; u < graph.U.size(); ++u) {
        for (int v : graph.U[u]) {
            ...
            if (edge_to_wedges.find(edge) != edge_to_wedges.end()) {
                for (const auto& [u1, u2] : edge_to_wedges[edge]) {
                    auto it = wedge_counts.find({u1, u2});
...
int main() {
    ...
    if (!dataDir.empty() && choice == 1) {
        ...
        try {
            graph = createUserGraph(uLabels, vLabels);
        } catch (const exception& e) {
